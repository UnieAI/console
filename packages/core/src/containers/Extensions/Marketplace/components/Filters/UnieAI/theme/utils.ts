...
import { KubedThemePalette } from '@kubed/components';
...
export const getColor = (
  color: keyof KubedThemePalette,
  theme: KubedTheme,
  defaultColor: keyof KubedThemePalette,
) => {
  const { palette } = theme;
  ...
  const colorNumberStr = color.replace(/[^0-9]/g, '');
  const colorNumber = colorNumberStr ? parseInt(colorNumberStr, 10) : 2;
