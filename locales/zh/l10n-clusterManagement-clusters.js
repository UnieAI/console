...
module.exports = {
  ...
  CLUSTER_DIRECT_IMPORT_TIP:
    'UWS 多集群控制平面通过提供的 kubeconfig 来直接连接导入集群，此种方式要求当前集群能够通过 kubeconfig 中的 server 地址直接访问待导入集群。 </br></br>通常适用于:</br>1. 当前集群和待导入集群在同一内网网络中</br>2. 当前集群和待导入集群已通过 VPN 或隧道等其它技术连通所在网络</br>3. kubeconfig 的 server 地址可以通过公网访问',
  CLUSTER_AGENT_IMPORT_TIP:
    'UWS 控制平面通过代理方式连接待导入集群，控制平面启动一个公开的代理服务，待导入集群创建相应的客户端组件连接代理服务，与控制平面之间建立一个反向代理。此种方式不需要待导入集群和控制平面在同一网络，也不要求待导入集群暴露集群的 apiserver 地址，但会有一定的网络性能损耗。</br></br>通常适用于:</br>1. 当前集群和待导入集群不在同一网络中<br/>2. 当前集群和待导入集群无法通过 VPN 或隧道等其它技术连通所在网络<br/>3. 对集群间网络性能损耗能容忍',
  ...
  CLUSTER_CONDITION_OPENPITRIXRUNTIMEREADY: '應用商店就绪',
  ...
  NO_CLUSTER_TIP_DESC: '集群是运行 UWS 的一组节点（物理或虚拟机）。',
  // List > Remove Cluster
  RISK_WARNING: '风险警告',
  REMOVE_CLUSTER_TIP_A: '集群被移除后，集群中原有的资源不会被自动清除。',
  REMOVE_CLUSTER_TIP_B:
    '集群被移除后，集群中原有的多集群配置数据不会被自动清除，卸载 UWS 或删除关联资源时可能会导致用户数据丢失。请务必参照 <a href="https://docs.kubesphere.com.cn/">UWS 官方文档</a>手动清理被移除集群中原有的多集群配置数据。',
